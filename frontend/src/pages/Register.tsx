...
export const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.response?.data?.errors) {
         ...
         const mensajes = err.response.data.errors.map((e: any) => e.message).join(' | ');
         setError(`Error: ${mensajes}`);
      } else {
         setError(err.response?.data?.message || 'Ocurrió un error al registrarse. Intenta de nuevo.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const RoleCard = ({ rol, icon: Icon, title, desc }: { rol: Rol, icon: any, title: string, desc: string }) => (
    <div 
      onClick={() => {
        setSelectedRol(rol);
        setError('');
    ...
    >
      <div className={`p-4 rounded-full transition-colors ${selectedRol === rol ? 'bg-primary-500 text-white' : 'bg-gray-100 text-gray-500'}`}>
        <Icon className="w-8 h-8" />
      </div>
      <div>
        <h3 className={`font-bold text-lg ${selectedRol === rol ? 'text-primary-800' : 'text-gray-700'}`}>{title}</h3>
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="w-full max-w-4xl">
        <div className="text-center mb-10">
          <h2 className="text-4xl font-extrabold text-gray-900 tracking-tight">Únete a EduPlay</h2>
        ...
        {selectedRol && (
          <div className="bg-white py-8 px-6 shadow-xl sm:rounded-xl sm:px-10 border border-gray-100 max-w-md mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            <h3 className="text-2xl font-bold text-center mb-6 text-gray-800">
              Registro como <span className="text-primary-600 capitalize">{selectedRol.toLowerCase()}</span>
            </h3>
              ...
              {(selectedRol === 'PROFESOR' || selectedRol === 'PADRE') && (
                <div className="animate-in fade-in slide-in-from-left-2">
                  ...
                  <input
                    ...
                    className="mt-1 appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500 sm:text-sm"
                  />
                  <p className="text-xs text-gray-500 mt-1">Por políticas de seguridad, debes ser mayor de 18 años.</p>
                </div>
              )}
